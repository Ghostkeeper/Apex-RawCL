...
const std::vector<SimplePolygonBenchmark> Benchmarker::device_benchmarks = {
	SimplePolygonBenchmark("area_opencl", [](const Device<>* device, SimplePolygon& polygon) {polygon.area_opencl(*device);}),
	SimplePolygonBenchmark("contains_opencl", [](const Device<>* device, SimplePolygon& polygon) {polygon.contains_opencl(*device, Point2(0, 0));})
};

const std::vector<SimplePolygonBenchmark> Benchmarker::host_benchmarks = {
	SimplePolygonBenchmark("area_host", [](const Device<>*, SimplePolygon& polygon) {polygon.area_host();}),
	SimplePolygonBenchmark("contains_host", [](const Device<>*, SimplePolygon& polygon) {polygon.contains_host(Point2(0, 0));})
};

Benchmarker::Benchmarker(const Device<>* device) : device(device) { }
...
int main(int, char**) {
	...
	for(const apex::Device<>& device : devices.getAll()) {
